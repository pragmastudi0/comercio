-- Fix de la RPC: corregir 3 errores de columnas/enums.
-- Esto reemplaza completamente la RPC anterior. Pegar en SQL Editor.
...
begin
  ...
  -- 1) Descontar stock SIN bloquear por insuficiencia (permite negativo)
  ...
  -- 3) Movimientos de stock (tipo 'venta')
  for v_item in select * from jsonb_array_elements(p_items) loop
  ...
  -- 4) Movimientos de caja por cada pago (tipo siempre 'venta')
  for v_pago in select * from jsonb_array_elements(p_pagos) loop
    ...
    -- 5) Si es cta_cte, cargar saldo del cliente
    if (v_pago->>'metodo') = 'cta_cte' and p_cliente_id is not null then
      ...
      values (
        p_cliente_id, 'cargo', (v_pago->>'monto')::numeric,
        v_venta.id, p_empleado_id, 'Venta ' || v_numero
      );
